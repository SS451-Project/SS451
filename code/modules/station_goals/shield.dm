...
/datum/station_goal/station_shield/get_report()
	return {"<b>Спутники с метеорными щитами</b><br>
	Станция расположена в зоне, полной космического мусора. У нас есть прототип системы защиты, которую вы развернете, чтобы уменьшить количество несчастных случаев, связанных с столкновениями.
	<br><br>
	Вы можете заказать спутники и системы управления через грузовой шаттл."}
...
/obj/machinery/computer/sat_control/proc/toggle(id)
	for(var/obj/machinery/satellite/S in GLOB.machines)
		if(S.id == id && atoms_share_level(src, S))
			if(!S.toggle())
				notice = "Вы можете активировать спутники, которые находятся только в космосе"
			else
				notice = null


/obj/machinery/satellite
	name = "Бездействующий спутник"
	desc = ""
...
/obj/machinery/satellite/proc/toggle(mob/user)
	if(!active && !isinspace())
		if(user)
			to_chat(user, "<span class='warning'>Вы можете активировать спутники, которые находятся только в космосе</span>")
		return FALSE
	if(user)
...
/obj/machinery/satellite/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/multitool))
		to_chat(user, "<span class='notice'>// NTSAT-[id] // Режим : [active ? "ОСНОВНОЙ" : "ОЖИДАНИЕ"] //[emagged ? "DEBUG_MODE //" : ""]</span>")
	else
		return ..()

/obj/machinery/satellite/meteor_shield
	name = "Спутник Метеоритного щита"
	desc = "Спутник защиты от Метеоритной зоны"
	mode = "M-SHIELD"
	speed_process = TRUE
	var/kill_range = 14
...
/obj/machinery/satellite/meteor_shield/emag_act(mob/user)
	if(!emagged)
		to_chat(user, "<span class='danger'>Вы отключаете схемы щита, заставляя его притягивать метеориты, вместо того чтобы уничтожать их.</span>")
		emagged = 1
		if(active)
			change_meteor_chance(2)
