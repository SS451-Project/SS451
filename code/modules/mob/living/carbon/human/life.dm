			...
			switch(health)
				...
				if(-79 to -50)
					...
					if(prob(health * -0.08))
						...
						ForceContractDisease(D)
					if(prob(6))
						to_chat(src, "<span class='userdanger'>Ты чувствуешь [pick("ужасную боль", "себя отвратительно", "себя как дерьмо", "себя абсолютно хуёво", "себя, как саму смерть", ", будто ты умираешь", "абсолютно ничего", "жар", "наступающий пот", "покалывание", "себя очень, очень плохо", "себя ужасно")]!</span>")
						Weaken(3)
					if(prob(3))
						Paralyse(2)
				if(-49 to 0)
					...
					if(prob(3))
						...
						ForceContractDisease(D)
					if(prob(5))
						to_chat(src, "<span class='userdanger'>Ты чувствуешь [pick("тяжесть", "себя отвратно", "себя как дерьмо", "себя больным", ", что онемел", "себя замёрзшим", "наступающий пот", "покалывание", "себя ужасно")]!</span>")
						Weaken(3)
